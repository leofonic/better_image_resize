...
if (!function_exists("copyAlteredImage")) {
    ...
    function copyAlteredImage($sDestinationImage, $sSourceImage, $iNewWidth, $iNewHeight, $aImageInfo, $sTarget, $iGdVer)
    {
        ...
        //fill destination image white
        $bg = imagecolorallocate($sDestinationImage, 255, 255, 255);
        imagefill($sDestinationImage, 0, 0, $bg);
        
        //correct ratio and calc offset
        if ($iNewWidth / $iNewHeight > $aImageInfo[0] / $aImageInfo[1]){
            $iDestWidth = $aImageInfo[0] / $aImageInfo[1] * $iNewHeight;
            $iDestHeight = $iNewHeight;
            $iDstY = 0;
            $iDstX = ($iNewWidth - $iDestWidth) / 2;
        }
        else{
            $iDestWidth = $iNewWidth;
            $iDestHeight = $aImageInfo[1] / $aImageInfo[0] * $iNewWidth;
            $iDstY = ($iNewHeight - $iDestHeight) / 2;
            $iDstX = 0;
        }
        
        //copy image centered
        if ($iGdVer == 1) {
            $blSuccess = imagecopyresized($sDestinationImage, $sSourceImage, $iDstX, $iDstY, 0, 0, $iDestWidth, $iDestHeight, $aImageInfo[0], $aImageInfo[1]);
        } else {
            $blSuccess = imagecopyresampled($sDestinationImage, $sSourceImage, $iDstX, $iDstY, 0, 0, $iDestWidth, $iDestHeight, $aImageInfo[0], $aImageInfo[1]);
        }
...
if (!function_exists("checkSizeAndCopy")) {
    ...
    function checkSizeAndCopy($sSrc, $sTarget, $iWidth, $iHeight, $iOrigWidth, $iOrigHeight)
    {
        ...
        //do not change desired size, image is placed centered in desired size in copyAlteredImage
        list($iNewWidth, $iNewHeight) = array($iWidth, $iHeight);
